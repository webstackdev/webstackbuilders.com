 ...
 */
const navigationSchema: FrontmatterSchema = {
  eleventyNavigation: {
    type: 'object',
    required: false,
    properties: {
      ...
      url: {
        type: 'string',
        description: `Allows adding a link to an external URL. Set 'permalink: false' to avoid generating a page for the template that the 'url' property is set on since it's an external link.`,
        format: 'url',
        required: false,
        messages: {
